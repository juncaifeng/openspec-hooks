...
const A1 = {
  ...
  check(ctx) {
    ...
    if (ctx.specsStagedPaths.length > 0 && ctx.archive.length === 0) {
      if (ctx.isBootstrap) {
        // fresh repo: the very first commit legitimately creates specs/
        return [
          f("A1", `初始提交引导 specs/（${ctx.specsStagedPaths.join(", ")}）— 豁免放行，此后 specs/ 只能随 archive 演进`, {
            severity: "info",
          }),
        ];
      }
      return [
        f("A1", `直接修改了 specs/（${ctx.specsStagedPaths.join(", ")}）而没有归档动作 — specs/ 是 source of truth，只能随 archive 合并 delta 演进`, {
...
const B5 = {
  ...
  check(ctx) {
    ...
    for (const c of ctx.changes) {
      ...
      for (const r of reqs) {
        if (!requirementCovered(r.name, tasks)) {
          out.push(
            f("B5", `需求 "${r.name}"（${r.from}）在 tasks.md 里找不到对应任务 — 有需求没排工`, {
              file: c.tasks.path,
              hint: "给每条 ADDED/MODIFIED 需求至少排一个实现/验证任务",
            })
          );
        }
      }
